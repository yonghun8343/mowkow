...
class KoreanArgumentParser(argparse.ArgumentParser):
    ...
    def _translate_message(self, message: str) -> str:
        ...
        if "unrecognized arguments:" in korean_message:
            korean_message = korean_message.replace(
                "unrecognized arguments:", "인식되지 않는 인수:"
            )
...
def main():
    ...
    envset(env, mksym("비트곱"), mkbuiltin(builtin_logand))
    envset(env, mksym("비트합"), mkbuiltin(builtin_logor))
    envset(env, mksym("베타합"), mkbuiltin(builtin_logxor))

    argparser = KoreanArgumentParser(description="머꼬 해석기", prog="mk")

    argparser.add_argument(
        "-o", "--output", dest="out_file", type=str, help="출력 파일명"
    )
